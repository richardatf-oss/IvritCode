// main.js
// Wiring for IvritCode VM (uses global window.IvritVM from vm.js).

(function () {
  const vm = window.IvritVM;
  if (!vm) {
    console.error("IvritVM not found on window.");
    return;
  }

  const { AO_INDEX, REGISTER_NAMES, runProgram } = vm;
  ...
  function renderTrace(result, traceListEl) {
    ...
    if (!executedOps || !trace || trace.length === 0) {
      traceListEl.innerHTML =
        "<li>No executable IvritCode letters (א–ת) were found.</li>";
      return;
    }
  ...
  function setup() {
    ...
    // Default example program if empty.
    ...
    // Initial render (all zeros, from running empty program)
    runOnce();
  }
